...
updateLotSqlServer: async (req, res) => {
    try {
     ...
     /* LotSQL: async (req, res) => {
      try {
          const { adminId, operationId } = req.params;
          ...
          console.log(" Données reçues du frontend:", data);

          const { code } = await lotService.addNewLotSQL(data);

          let messages = {
              0: "Lot ajouté avec succès.",
              2001: "Opération introuvable.",
              2002: "Administrateur introuvable.",
              2003: "Ce lot existe déjà.",
  ...
  GetAllLotsSQL: async (req, res) => {
    try {
      const lots = await lotService.getAllLotsSQL();
      res.status(200).json({
        success: true,
        data: lots
      });
    } catch (error) {
      console.error(" Controller error (GetAllLotsSQL):", error);
      res.status(500).json({
        success: false,
        message: "Erreur serveur : " + error.message
      });
