...
module.exports = {
  ...
  romoveAnnonceSQL: async (req, res) => {
    try {
      const { Numero } = req.params;
      console.log('Requête suppression annonce numéro :', Numero);

      const result = await annonceService.deleteAnnonceSQL(Numero);

      if (result.code === 0) {
        res.status(200).json({
          success: true,
          message: `Annonce ${Numero} supprimée avec succès.`,
          code: result.code
        });
      } else if (result.code === 1003) {
        res.status(404).json({
          success: false,
          message: `Annonce ${Numero} introuvable ou déjà supprimée.`,
          code: result.code
        });
      } else {
        res.status(500).json({
      ...
      console.error('Controller error (DeleteAnnonceSQL):', error);
      res.status(500).json({
        success: false,
        message: 'Erreur serveur: ' + error.message,
      });
