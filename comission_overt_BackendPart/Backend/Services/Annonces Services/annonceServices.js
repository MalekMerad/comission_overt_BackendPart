...
module.exports = {
  ...
  deleteAnnonceByIdSqlServer: async (id) => {
    ...
    getAllAnnonces: async () => {
        try {
        const [rows] = await db.query('CALL getAllANNONCESSQL()');
        return rows[0] || [];
        } catch (error) {
        console.error('Service error (getAllAnnonces):', error);
        throw error;
        }
    },

    deleteAnnonceSQL: async (Numero) => {
        try {
            await db.query('CALL DeleteAnnonceSQL(?, @resultCode)', [Numero]);
            const [rows] = await db.query('SELECT @resultCode AS code');
            const resultCode = rows[0]?.code ?? null;
