...
updateLotSqlServer: async (id_Lot, lot_Designation) => {
  ...
  /*
  addNewLotSQL: async (data) => {
    const { NumeroLot, Designation, id_Operation, adminId } = data;

    console.log('📦 Données envoyées au service Lot:', data);
    ...
    try {
      ...
      console.log(' Résultat MySQL (insertNewLotSQL):', resultCode);
      return { code: resultCode };

    } catch (error) {
      console.error(' Service error (addNewLotSQL):', error);
      throw error;
    }
  },
  ...
  getAllLotsSQL: async () => {
    try {
      const [rows] = await db.query('CALL getAllLotsSQL()');
      console.log(' Lots récupérés :', rows);
      return rows[0];
    } catch (error) {
      console.error(' Service error (getAllLotsSQL):', error);
      throw error;
    }
  },
