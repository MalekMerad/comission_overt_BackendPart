...
module.exports = {
    ...
    getAllOperationsSQL: async () => {
        try {
            const [rows] = await db.query('CALL getAllOPERATIONSSQL()');
            const operations = Array.isArray(rows[0]) ? rows[0] : rows;
            return operations || [];
        } catch(error) {
            console.error('Service error (getAllOperationsSQL):', error);
            throw error;
        }
    },
